...
import com.stj.fileexplorer.FileListItem.ModeCallback;
import com.stj.fileexplorer.FileOperationHelper.IOperationProgressListener;
import com.stj.fileexplorer.FileSortHelper.SortMethod;
import com.stj.fileexplorer.FileViewActivity.SelectFilesCallback;
import com.stj.fileexplorer.TextInputDialog.OnFinishListener;
import com.stj.fileexplorer.Util.SDCardInfo;
...
public class FileViewInteractionHub implements IOperationProgressListener {
    ...
    private boolean IS_LIMIT_VIEW_PATH = false;
    private String LIMITED_ROOT_PATH;
    private boolean FUSE_LIMITED_PATH = false;
    private boolean mIsFileViewActivity = false;
    ...
    public FileViewInteractionHub(IFileInteractionListener fileViewListener) {
        ...
        setup();
        mFileOperationHelper = new FileOperationHelper(this, mFileViewListener.getContext());
        mFileSortHelper = new FileSortHelper();
        mContext = mFileViewListener.getContext();

        IS_LIMIT_VIEW_PATH = Util.isLimitViewPath(mContext);
        LIMITED_ROOT_PATH = Util.getLimitedRootPath(mContext);
        FUSE_LIMITED_PATH = Util.isFuseLimitedRootPath(mContext);
        mIsFileViewActivity = mFileViewListener instanceof FileViewActivity;
    }
    ...
    private OnClickListener navigationClick = new OnClickListener() {
        ...
        public void onClick(View v) {
            ...
            assert (path != null);
            if (FUSE_LIMITED_PATH && "".equals(path)) {
                path = LIMITED_ROOT_PATH;
            }
    ...
    protected void onNavigationBarClick() {
        if (mDropdownNavigation.getVisibility() == View.VISIBLE) {
            ...
            if (IS_LIMIT_VIEW_PATH
                    && mIsFileViewActivity) {

                if (FUSE_LIMITED_PATH) {
                    ...
                    text.setText(Util.FUSE_ROOT_PATH);
                ...
                HashMap<String, String> volumeMap = Util.getVolumeMap(mContext);
                for (String path : volumeMap.keySet()) {
                    if (!mCurrentPath.startsWith(path)) {
                        View listItem = LayoutInflater.from(mContext).inflate(
                                R.layout.dropdown_item,
                                null);
                        ...
                        img.setImageResource(
                                FUSE_LIMITED_PATH ?
                                        ...
                                        : R.drawable.dropdown_icon_root);

                        TextView text = (TextView) listItem.findViewById(R.id.path_name);
                        String display = mFileViewListener.getDisplayPath(path);
                        if (FUSE_LIMITED_PATH && display.startsWith(Util.FUSE_ROOT_PATH)) {
                            display = display.replaceFirst(Util.FUSE_ROOT_PATH, "");
                        }
                        text.setText(mFileViewListener.getDisplayPath(display));

                        listItem.setOnClickListener(navigationClick);
                        listItem.setTag(path);
                        list.addView(listItem);
                    }
                }
            }
            ...
            while (pos != -1 &&
                    !displayPath.equals(IS_LIMIT_VIEW_PATH ? LIMITED_ROOT_PATH : "/")) {//如果当前位置在根文件夹则不显示导航条
                ...
                if (root == true
                        && IS_LIMIT_VIEW_PATH
                        && mIsFileViewActivity
                        && FileExplorerPreferenceActivity.showRealPath(mContext)) {
                    HashMap<String, String> volumeMap = Util.getVolumeMap(mContext);
                    for (String volumePath : volumeMap.keySet()) {
                        if (displayPath.startsWith(volumePath)) {
                            end = volumePath.length();
                            break;
                        }
                    }
                }
    ...
    public boolean onOperationUpLevel() {
        ...
        if (IS_LIMIT_VIEW_PATH) {
            if (mCurrentPath != null
                    && mCurrentPath.startsWith(
                            LIMITED_ROOT_PATH + (FUSE_LIMITED_PATH ? "" : File.separator))) {
                String parentPath = new File(mCurrentPath).getParent();
                if (LIMITED_ROOT_PATH.equals(
                        FUSE_LIMITED_PATH ? mCurrentPath : parentPath)) {
                    return false;
                }
        ...
        if (!GlobalConsts.ROOT_PATH.equals(mCurrentPath)) {
    ...
    private boolean doCreateFolder(String text) {
        ...
        if (TextUtils.isEmpty(text)) {
            Util.showPrompt(mContext, R.string.prompt_folder_name_not_empty);
            return false;
        }
    ...
    public void onOperationSearch() {
        //add by xugaoming@20140517, add search function
    ...
    private void onOperationPaste() {
        ...
        mFileOperationHelper.Paste(mCurrentPath);
    }
    ...
    private void updateNavigationPane() {
        ...
        boolean isHideUpLevel = false;
        if (IS_LIMIT_VIEW_PATH) {
            HashMap<String, String> volumeMap = Util.getVolumeMap(mContext);
            if (FUSE_LIMITED_PATH) {
                isHideUpLevel = LIMITED_ROOT_PATH.equals(mCurrentPath);
            } else {
                isHideUpLevel = volumeMap.containsKey(mCurrentPath);
            }
        } else {
            isHideUpLevel = GlobalConsts.ROOT_PATH.equals(mCurrentPath);
        }
        upLevel.setVisibility(isHideUpLevel ? View.INVISIBLE : View.VISIBLE);
        arrow.setVisibility(isHideUpLevel ? View.GONE : View.VISIBLE);
        ...
        for (FileInfo f : selectedFileList) {
            if (f.IsDir) {
                AlertDialog dialog = new AlertDialog.Builder(mContext).setMessage(
                        R.string.error_info_cant_send_folder)
    ...
    public void onOperationRename() {
        mRenameSelectedItem = mListViewContextMenuSelectedItem;
        int pos = mListViewContextMenuSelectedItem;
        if (pos == -1)
            return;
    ...
    private boolean doRename(final FileInfo f, String text) {
        ...
        if (mFileOperationHelper.Rename(f, text)) {
            f.fileName = text;
            f.filePath = Util.makePath(Util.getPathFromFilepath(f.filePath), text);
            mFileOperationHelper.onOperationFinished();
            mFileViewListener.onDataChanged();
            mFileListView.post(new Runnable() {
                @Override
                public void run() {
                    mFileListView.setSelection(mRenameSelectedItem);
                    mRenameSelectedItem = 0;
                }
            });
        } else {
            new AlertDialog.Builder(mContext)
    ...
    private void notifyFileSystemChanged(String path) {
        ...
        final Intent intent;
        if (f.isDirectory()) {
            intent = new Intent(ACTION_MEDIA_SCANNER_SCAN_ALL);
    ...
    public void onSetRingtone()
    {
        ...
        Intent intent = new Intent("android.intent.action.GOSO_SET_RINGTONE");
        intent.putExtra("path", file.filePath);
        mContext.sendBroadcast(intent);
        clearSelection();
    }
    public void onOperationButtonConfirm() {
        if (mFileOperationHelper.isLimitViewPath(mContext, mCurrentPath)) {
            new AlertDialog.Builder(mContext)
                    .setMessage(mContext.getString(R.string.fail_to_paste))
                    .setPositiveButton(R.string.confirm, null).create().show();
            return;
        }

        if (!hasSufficientSpace()) {
            Util.showPrompt(mContext, R.string.prompt_paste_fail_no_space);
            return;
        }
    ...
    private OnCreateContextMenuListener mListViewContextMenuListener = new OnCreateContextMenuListener() {
        ...
        public void onCreateContextMenu(ContextMenu menu, View v, ContextMenuInfo menuInfo) {
            ...
            boolean isShowFileOperation = true;
            final boolean isFuseLimitedRoot = Util.isFuseLimitedRootPath(v.getContext());
            final HashMap<String, String> volumeMap = Util.getVolumeMap(v.getContext());
            if (isFuseLimitedRoot
                    && file != null && volumeMap.containsKey(file.filePath)) {
                isShowFileOperation = false;
            }
            ...
            {
                MediaFile.MediaFileType type = MediaFile.getFileType(file.filePath);
                if (type != null && MediaFile.isAudioFileType(type.fileType))
                {
                    addMenuItem(menu, MENU_SET_AS_RINGTONE, 0, R.string.operation_set_ringtone);
                }
            }

            addMenuItem(menu, GlobalConsts.MENU_COPY_PATH, 0, R.string.operation_copy_path);

            if (isShowFileOperation) {
                addMenuItem(menu, GlobalConsts.MENU_COPY, 0, R.string.operation_copy);
            }
            /*if (isShowFileOperation) {
                addMenuItem(menu, GlobalConsts.MENU_PASTE, 0, R.string.operation_paste);
            }*/
            if (isShowFileOperation) {
            ...
            if (menuItem != null) {
                if (!canPaste() || !isShowFileOperation) {
    ...
    private int mRenameSelectedItem;
    ...
    private static final int MENU_SET_AS_RINGTONE = 19;
    ...
    private com.stj.fileexplorer.FileViewInteractionHub.Mode mCurrentMode;
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        ...
        addMenuItem(menu,MENU_SEARCH, 0, R.string.menu_item_search);
        addMenuItem(menu, MENU_SELECTALL, 0, R.string.operation_selectall,
                R.drawable.ic_menu_select_all);

        addMenuItem(menu, MENU_SORT, 1, R.string.menu_item_sort,
    ...
    private void updateMenuItems(Menu menu) {
        final boolean isFuseLimitedRoot = Util.isFuseLimitedRootPath(mContext);
        final String limitedRootPath = Util.getLimitedRootPath(mContext);
        boolean isHideFileOperation = isFuseLimitedRoot && limitedRootPath.equals(mCurrentPath);
        MenuItem newFolderItem = menu.findItem(GlobalConsts.MENU_NEW_FOLDER);
        if (newFolderItem != null) {
            newFolderItem.setVisible(!isHideFileOperation);
        }
    ...
    // add begin by lzy@20140303,for detect free block before paste.
    private boolean hasSufficientSpace() {
        ArrayList<FileInfo> files = mFileOperationHelper.getFileList();
        long fileSize = Util.getFileSize(files);
        SDCardInfo info = Util.getVolumeInfo(mCurrentPath);
        if (info != null) {
            Log.i(LOG_TAG, "free size=" + info.free + " " + "file size=" + fileSize);
            if (info.free < fileSize) {
                return false;
            }
        }
        return true;
    ...
    private void onOperationSort() {
        final int ITEM_SORT_DATE = 0;
        final int ITEM_SORT_NAME = 1;
        final int ITEM_SORT_SIZE = 2;
        final int ITEM_SORT_TYPE = 3;
        AlertDialog.Builder builder = new AlertDialog.Builder(mContext);
        builder.setTitle(R.string.menu_item_sort)
                .setItems(mContext.getResources().getStringArray(R.array.sort_by),
    ...
    public void refreshFileInteraction() {
        mFileViewListener.onRefreshFileInteraction();
    }

    public boolean isFileOperationFinished() {
        return isOperationFinished;
    }

    public void setOperationFinished(boolean isOperationFinished) {
        this.isOperationFinished = isOperationFinished;
    }
    // add end.

    public void updateListViewSelectedItemPosition(int pos) {
