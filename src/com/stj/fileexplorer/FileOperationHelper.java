...
public class FileOperationHelper {
    ...
    public boolean CreateFolder(String path, String name) {
        ...
        //modify begin by xugaoming@20140519,
        //notify mediaprovider about the new dir
        boolean ret =  f.mkdir();
        if (ret)
        {
        ...
        asnycExecute(new Runnable() {
            ...
            public void run() {
                for (FileInfo f : mCurFileNameList) {
                    ...
                    mOperationListener.onFileChanged(_path);
                }
                clear();
            }
        });
    ...
    public boolean isLimitViewPath(Context context, String path) {
        final boolean isLimitViewPath = Util.isLimitViewPath(context);
        final String limitViewPath = Util.getLimitedRootPath(context);
        return isLimitViewPath && limitViewPath.equals(path);
    }
    ...
    private void asnycExecute(Runnable r) {
        ...
        new AsyncTask() {
            ...
            protected Object doInBackground(Object... params) {
                // add by xugaoming@20140312, aquire wakelock before operation
    ...
    public boolean Rename(FileInfo f, String newName) {
        ...
        if (new File(newPath).exists() && (!f.fileName.equals(newName.trim()))) {
            return false;
        }
    ...
    public boolean Delete(ArrayList<FileInfo> files) {
        ...
        asnycExecute(new Runnable() {
            ...
            public void run() {
                for (FileInfo f : mCurFileNameList) {
                    DeleteFile(f);
                    // mOperationListener.onFileChanged(f.filePath);//delete by lzy@20140307,for immediately
                    // scan file system after delete file.
                }
                clear();
            }
        });
    ...
    protected void DeleteFile(FileInfo f) {
        ...
        file.delete();
        // modify begin by  xiaoye@20141117 : remove the related Data from
        ...
        if (databaseHelper != null && databaseHelper.isFavorite(f.filePath)) {
            databaseHelper.delete(f.filePath);
        }
        //modify end
        mOperationListener.onFileChanged(f.filePath);// add by lzy@20140307.for immediately scan file
                                                     // system after delete file.
    ...
    private void CopyFile(FileInfo f, String dest) {
        ...
        if (file.isDirectory()) {
            ...
            File newDir = new File(destFile.getAbsolutePath());
            newDir.mkdirs();
            for (File child : file.listFiles()) {
                if (!child.isHidden() && Util.isNormalFile(child.getAbsolutePath())) {
                    CopyFile(Util.GetFileInfo(child, mFilter, Settings.instance().getShowDotAndHiddenFiles()), newDir.getPath());
                }
            }
        } else {
