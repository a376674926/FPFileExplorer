...
import com.stj.fileexplorer.FavoriteDatabaseHelper.FavoriteDatabaseListener;
import com.stj.fileexplorer.FileCategoryHelper.CategoryInfo;
import com.stj.fileexplorer.FileCategoryHelper.FileCategory;
...
public class FileCategoryActivity extends Fragment implements IFileInteractionListener,
    ...
    private boolean isExternalMount = false;

    private String mReceiveAction = null;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        // modify by zhongrenzhan@20140134,add file pick mode for
        ...
        // modify end
        mFileViewInteractionHub.setDefaultPath("/");
        mFileIconHelper = new FileIconHelper(mActivity);
        mFavoriteList = new FavoriteList(mActivity,
                (ListView) mRootView.findViewById(R.id.favorite_list), this, mFileIconHelper);
        mFavoriteList.initList();
        mAdapter = new FileListCursorAdapter(mActivity, null, mFileViewInteractionHub,
                mFileIconHelper);
    ...
    public void refreshCategoryInfo() {
        ...
        String defaultStorage = Util.getDefaultExternalStorage(mActivity);
        SDCardInfo storageInfo = Util.getVolumeInfo(defaultStorage);
        if (defaultStorage != null && storageInfo != null) {
            String descriptions = volumeMap.get(defaultStorage);
            mCategoryBar.setFullValue(storageInfo.total);
            setTextView(R.id.sd_card_capacity,
                    getString(R.string.storage_size,
                            descriptions,
                            Util.convertStorage(storageInfo.total)));
            setTextView(R.id.sd_card_available,
                    getString(R.string.storage_available,
                            Util.convertStorage(storageInfo.free)));
        }
        ...
        if (storageInfo != null) {
            long otherSize = storageInfo.total - storageInfo.free - size;
    ...
    private void showPage(ViewPage p) {
        ...
        showView(R.id.sd_not_available_page_category, false);
        ...
        switch (p) {
            ...
            case Category:
                ...
                break;
            case NoSD:
                showView(R.id.sd_not_available_page_category, true);
                break;
        }
    }

    private void showEmptyView(boolean show) {
        View emptyView = mActivity.findViewById(R.id.empty_view_category);
        if (emptyView != null)
    ...
    private void showView(int id, boolean show) {
        ...
        if ((id == R.id.sd_not_available_page_category) && show) {
            showEmptyView(false);
        }
    }
    ...
    private void onCategorySelected(FileCategory f) {
        if (mFileCagetoryHelper.getCurCategory() != f) {
            ...
            mFileViewInteractionHub.setCurrentPath(mFileViewInteractionHub.getDefaultPath()
    ...
    public boolean onRefreshFileList(String path, FileSortHelper sort) {
        ...
        if (bar != null) {
            ...
            if ((mFileViewInteractionHub != null)
                    ...
                    && (selectedNavigationIndex == Util.CATEGORY_TAB_INDEX))) {
                mFileViewInteractionHub.refreshFileInteraction();
            }
        }
        refreshFileList();
        if (FileCategory.All != mFileCagetoryHelper.getCurCategory()) {
            if (timer != null) {
                timer.cancel();
            }
            timer = new Timer();
            timer.schedule(new TimerTask() {
                public void run() {
                    handler.sendEmptyMessage(MSG_FILE_CHANGED_REFRESH);
                }
    ...
    private boolean refreshFileList() {
        ...
        Cursor c = mFileCagetoryHelper.query(curCategory,
                mFileViewInteractionHub.getFileSortHelper().getSortMethod());
    ...
    public void onPick(FileInfo f) {
        // add by zhongrenzhan@20140124,return the result for the launch
        // activity.
        if (null == f) {
            return;
        }
    ...
    private class ScannerReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            Log.v(LOG_TAG, "received broadcast: " + mReceiveAction.toString());
            // handle intents related to external storage
            if (mReceiveAction.equals(Intent.ACTION_MEDIA_SCANNER_FINISHED)
                    || mReceiveAction.equals(Intent.ACTION_MEDIA_MOUNTED)
                    || mReceiveAction.equals(Intent.ACTION_MEDIA_UNMOUNTED)
                    || mReceiveAction.equals(Intent.ACTION_MEDIA_BAD_REMOVAL)) {
                if (!Intent.ACTION_MEDIA_SCANNER_FINISHED.equals(mReceiveAction)) {
                    showPage(ViewPage.Home);
                }
                notifyFileChanged();
            }
    ...
    private void updateUI() {
        ...
        if (sdCardReady) {
            if (isExternalMount != Util.isExternalStorageReady(mActivity)) {
                isExternalMount = Util.isExternalStorageReady(mActivity);
                showPage(ViewPage.Home);
            }
            ...
            // Modify by lzy@20140317,for do not clear check box after media
            // scan finished.
    ...
    private static final int MSG_FILE_CHANGED_REFRESH = 101;
    // add begin by xiaoye@20141117: remove the related Data from
    // fravotite database after delete the fravotite file.
    private static final int MSG_FAVORITE_CHANGED = 102;
    //add end
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                case MSG_FILE_CHANGED_TIMER:
                    updateUI();
                    break;
                case MSG_FILE_CHANGED_REFRESH:
                    refreshCategoryInfo();
                    refreshFileList();
                    break;
                // add begin by xiaoye@20141117: remove the related Data from
                // fravotite database after delete the fravotite file.
                case MSG_FAVORITE_CHANGED:
                    setCategoryCount(FileCategory.Favorite, mFavoriteList.getCount());
                    break;
                // add end
            }
    ...
    public void onFavoriteDatabaseChanged() {
        //modify begin by xiaoye: remove the related Data from
        // fravotite database after delete the fravotite file.
        Message message = new Message();
        message.what = MSG_FAVORITE_CHANGED;
        handler.sendMessage(message);
        //modify end
    }
    ...
    public void onRefreshFileInteraction() {
        Log.i(LOG_TAG, "auto refresh FileViewActivity");
        mFileViewActivity.refresh();
        if (mFileViewInteractionHub != null) {
