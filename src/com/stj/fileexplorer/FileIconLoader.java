...
public class FileIconLoader implements Callback {
    ...
    private void requestLoading() {
        if (!mLoadingRequested) {
            ...
            mMainThreadHandler.sendEmptyMessageDelayed(MESSAGE_REQUEST_LOADING, 200);
        }
    }
    ...
    private class LoaderThread extends HandlerThread implements Callback {
                ...
                if (holder != null && holder.state == ImageHolder.NEEDED) {
                    ...
                    switch (id.mCategory) {
                        ...
                        case Video:
                            ...
                            if (id.mId == 0) {
                                Log.e(TAG, "Fail to get dababase id for:" + id.mPath);
                            }
                            holder.setImage(isVideo ? getVideoThumbnail(id.mPath, MICRO_KIND) : getImageThumbnail(id.mPath));
                            break;
                    }
        ...
        // deleted begin by lzy@20140606,for get image thumbnail more faster.
        /*private Bitmap getImageThumbnail(long id) {
            Bitmap thumbNail;
            try {
                thumbNail = Images.Thumbnails.getThumbnail(mContext.getContentResolver(), id,
                        MICRO_KIND, null);
            } catch (IllegalStateException e) {
                thumbNail = BitmapFactory.decodeResource(mContext.getResources(),
                        R.drawable.file_icon_picture);
                Log.e(TAG, "Fail to get Image Thumbnail:" + e.toString());
            }
            return thumbNail;
        }*/
        // deleted end.

        // add begin by lzy@20140929,for decode video thumbnail every time,don't
        // qury the thumbnail from mediaStore cache,because when the cache don't
        // clean and the video ID same as before,the thumbnail will wrong.
        private Bitmap getVideoThumbnail(String path, int kind) {
            Bitmap thumbNail;
            thumbNail = ThumbnailUtils.createVideoThumbnail(path, kind);
            if (thumbNail == null) {
                thumbNail = BitmapFactory.decodeResource(mContext.getResources(),
                        R.drawable.file_icon_video);
                Log.e(TAG, "Fail to get Video Thumbnail set default video icon");
            }
            return thumbNail;
        }
        // add end.

        // add begin by lzy@20140606,for get image thumbnail more faster.
        private Bitmap getImageThumbnail(String path) {
            Bitmap thumbNail;
            BitmapFactory.Options options = new BitmapFactory.Options();
            options.inJustDecodeBounds = true;
            // Decode the width and height of the bitmap, but don't load the
            // bitmap to RAM
            BitmapFactory.decodeFile(path, options);
            int size = computeSampleSize(options, -1, 128 * 128);
            if (size <= 0) {
                size = 1;
            }
