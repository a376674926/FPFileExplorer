...
public class IntentBuilder {
    ...
    public static void viewFile(final Context context, final String filePath) {
        ...
        if (!TextUtils.isEmpty(type) && !TextUtils.equals(type, "*/*")) {
            ...
            // add begin by xugaoming@20140317, .3gp file may be audio file,
            // need to check it deeper by querying mediaprovider
            if ("video/3gpp".equals(type))
            {
                if ("audio/3gpp".equals(getMimeTypeDeeply(context, filePath)))
                {
                    type = "audio/3gpp";
                }
            }
    ...
    // add begin by xugaoming@20140317, query mime from mediaprovider
    private static String getMimeTypeDeeply(Context context, final String path)
    {
        String where = MediaStore.Files.FileColumns.DATA + "=\"" + path + "\"";
